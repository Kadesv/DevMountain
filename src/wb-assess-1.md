...
```
a console log is just visible in the console. Return is given to send code back to where the function was invoked.
```

**_What's the difference between defining and calling a function?_**

```
defining is writing the code and what it does. calling is putting values into it and making it run.
```
...
```
A string is a type of value that can be basically anything as long as it's surrounded by quotation marks.
```

**_Number_**

```
Number is a strictly numerical data type.
```
...
```
An array is a container that can hold a as many values as you may need.
```
